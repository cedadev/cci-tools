...
def process_file(line, count, total_files):
    ...
    for ref in dsr["refs"].keys():
        if len(dsr["refs"][ref]) == 3:
            dsr["refs"][ref][0] = dsr["refs"][ref][0].replace(
    ...
    for fmt in [
        ...
        try:
            tcs = datetime.strftime(
                datetime.strptime(ds.time_coverage_start, fmt), "%Y-%m-%dT%H:%M:%S"
            )
            tce = datetime.strftime(
                datetime.strptime(ds.time_coverage_end, fmt), "%Y-%m-%dT%H:%M:%S"
            )
        except:
            pass
    ...
    try:
        ...
        stac_record["properties"]["platforms"] = stac_record["properties"].pop(
            "platform"
        )

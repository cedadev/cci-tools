...
def main():
    ...
    slack_client, slack_cfg = None, None
    if os.path.isfile(
        "/home/users/dwest77/cedadev/cci/cci-tools/config/slack_cfg.json"
    ):
        with open(
            "/home/users/dwest77/cedadev/cci/cci-tools/config/slack_cfg.json"
        ) as f:
    ...
    if slack_cfg is not None:
        ...
        logger.info('Obtained slack config')
    ...
    for service, url in OTC_SERVICES.items():
        ...
        logger.info(f"{service} : {sitrep} ({r})")
        msg.append(sitrep)
    ...
    if slack_client is not None:
        slack_client.chat_postMessage(
            channel=channel, text="\n".join(message), username=f"CCI Service Sweeper"
        )

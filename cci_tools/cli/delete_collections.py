...
def recursive_removal(
    ...
    remove_items(
        f"{collection}/items", dryrun=dryrun, item_aggregations=item_aggregations
    )
...
DEPTHS_EXPLAINED = ["CCI", "Project", "Moles-Record", "DRS"]
...
@click.option(
    "--keep_collections", "keep_collections", is_flag=True, help="Keep collections"
)
@click.option(
    "--item_aggregations",
    "item_aggregations",
...
@click.option(
    "--top_only", "top_only", is_flag=True, help="Delete the specified collection only"
)
@click.option(
    "--lowest_only",
    "lowest_only",
...
@click.option("-r", "realrun", is_flag=True, help="Actually delete content")
@click.option(
    "--delete_depth",
    "delete_depth",
    type=int,
...
def main(
    ...
    """
    Delete STAC objects at the collection/item level.

    Options are available to specify the depth of deletion, whether to keep collections,
    and whether to delete aggregated items. By default, this will delete the specified
    collection and all collections/items below it - but only if the `realrun` flag is set,
    otherwise it will perform a dry run and print the actions that would be taken."""
    ...
    if not dryrun:
        if keep_collections:
            print(
                f"Are you sure you want to remove all items from the {collection} collection?"
            )
        elif top_only:
            ...
            print(
                f"Are you sure you want to remove all collections at the {exp} level?"
            )
        else:
            print(
                f"Are you sure you want to remove the {collection} collection and ALL collections/items below it?"
            )
        ask = input("Y/N: ")
        if ask != "Y":
            return
